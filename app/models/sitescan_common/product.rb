    ...
    scope :not_disabled, ->{
      ...
      scope :catalog, -> (category_ids) {
        ...
        def product_data(options)
          ...
          if options
            search_product_ids = nil
            options.each do |k,v|
              sp_ids = SitescanCommon::AttributeOption
                .where(attribute_class_option_id: v)
                .joins(:product_attribute)
                .pluck 'product_attributes.attributable_id'
              if search_product_ids.blank?
                search_product_ids = sp_ids
              else
                search_product_ids = search_product_ids & sp_ids
              end
            end
            price = price.where search_products: {id: search_product_ids}
          end
          ...
          acc: {category_id: categories.ids}).map do |attr|
              ...
              # Product attribute ids linked to search product.
              search_product_ids = search_products.joins(:product_attributes)
              if options
                opts = options.clone
                opts.delete attr.id.to_s
                search_product_ids = search_product_ids
                  .joins(%{JOIN attribute_options ao
                ON ao.id=product_attributes.value_id AND
                product_attributes.value_type='#{SitescanCommon::AttributeOption}'})
                opts.each do |k, v|
                  search_product_ids = search_product_ids
                    .where(ao: {attribute_class_option_id: v})
                end
              end
              search_product_ids = search_product_ids.pluck 'search_products.id'

              # Class option ids linked to search product.
              attr_cls_opt_ids = SitescanCommon::AttributeOption
                .joins(:product_attribute)
                .where(product_attributes: {attributable_id: search_product_ids,
              attributable_type: SitescanCommon::SearchProduct})
